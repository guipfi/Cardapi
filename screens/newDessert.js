...
export default function NewDessert({navigation}){
    const restaurant = useSelector(state => state.user);
    const myRef = firebase.database().ref("restaurant/"+restaurant.id+"/cardapio/sobremesas").push();
    const key = myRef.key
    ...
    const pickImage = async () => {
      ...
      const UserSchema  = yup.object({
          name: yup.string().required('Digite um nome para o sua sobremesa').min(2,'Digite um nome maior'),
          desc: yup.string().required('Digite uma descrição para o sua sobremesa').min(5, "a sobremesa deve ter uma descrição de no mínimo 5 caracteres."),   
          price: yup.number().required('A sobremesa deve possuir um preço')
        })
